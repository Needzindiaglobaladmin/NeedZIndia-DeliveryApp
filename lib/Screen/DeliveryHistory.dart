...
class DeliveryHistory extends StatefulWidget {
  @override
  _DeliveryHistory createState() => _DeliveryHistory();
}

class _DeliveryHistory extends State<DeliveryHistory> {
  bool loading = false;
  bool load = true;
  int offset =0;
  ...
  loadingDetails() async {
    try{
      ...
      var response = await deliveryDetails.GetDeliveryItems(startDate,endDate,offset);
  ...
  loadingOffset(offset) async {
    try{
      ...
      var ending =  now.subtract(Duration(days: 1));
      var endDate = formatter.format(ending);
      var response = await deliveryDetails.GetDeliveryItemsOffset(startDate,endDate,offset);
      if(response.success == true){
        if(mounted){
          setState(() {
            loading = false;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: FittedBox(fit:BoxFit.fitWidth,
          child:Container(
            child:  Text('Delivery History', style: GoogleFonts.quicksand()),
          ),
        ),
